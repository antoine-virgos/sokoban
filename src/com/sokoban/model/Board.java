...
public class Board {
    ...
    private void drawContent(int i) throws BoardCellException {
        for (int j = 0; j <= getSizeY(); j++) {
            ...
            if (ce != null) {
                Item item = ce.getItem();

                // wall
                if (item.equals(Item.WALL)) {
                    OUT.print(Item.WALL);
                }
                // player
                if (item.equals(Item.PLAYER)) {
                    OUT.print(Item.PLAYER);
                }
                // crate
                if (item.equals(Item.CRATE)) {
                    OUT.print(Item.CRATE);
                }
                // target
                if (item.equals(Item.TARGET)) {
                    OUT.print(Item.TARGET);
                }
                // void
                if (item.equals(Item.VOID)) {
                    OUT.print(Item.VOID);
                }
            }
     ...
     * not valid
     */
    public void drawBoard() throws BoardCellException {
        drawOrdinateY();
        drawAbscissaX();
    }
}